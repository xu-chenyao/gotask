要求
使用go-zero框架，开发一个人博客系统的后端， [] 实现一个Http的网关服务 [] 实现RPC服务（user、blog） [] 通过注册中心（ETCD）完成服务注册发现，实现博客文章的基本管理功能，包括文章的创建、读取、更新和删除（CRUD）操作，同时支持用户认证和简单的评论功能。 这个帮我按照项目建设和开发步骤从0-1 搭建 给我每一步的详细搭建和测试流程
